...
export default class Mino {
    ...
    public processMinoEvent(board: Board, squares: BoardSquares, event: MinoEvent): BoardSquares {
        switch (event) {
            case MinoEvent.drop: return this.drop(board, squares);
            case MinoEvent.rotateLeft: return this.rotateLeft(board, squares);
            case MinoEvent.rotateRight: return this.rotateRight(board, squares);
